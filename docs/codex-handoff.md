...
Last updated: 2026-08-22 19:00:00
...
9. 162条双突的远程NetSolP/NanoMelt评分和结果联接已完成，矩阵门为pass。相对WT，U/S/Tm原始值为正的候选分别有93/125/106条；达到中等或明显有利档的分别有31/35/54条，中等或明显不利的分别有19/2/14条。连同AntiFold组成位点最差档一起审核后，100条不含中等/明显不利项，其中84条至少有一个工具家族达到中等/明显有利，5条同时获得NetSolP家族、NanoMelt和AntiFold三个家族支持。组合预测不可直接按单突相加：U/S双突值与组成单突加和值的Spearman仅为0.579/0.660，而Tm为0.995；因此终选11条必须使用真实双突U/S/Tm结果，不能只按父单突分数拼接。当前尚未选择11条。
...
- `double_mutant_enumeration_release=pass`：162条不同位置双突、9条同位点互斥审计、完整序列风险/稳定词证据及163条共同评分样本已经生成；尚未执行性质评分或11条终选。
...
- `final_19_single_plus_11_double_panel_release=blocked`：只等待从完整162行矩阵按预先冻结的幅度、风险和多样性规则选择11条双突并生成最终19+11序列交付。
