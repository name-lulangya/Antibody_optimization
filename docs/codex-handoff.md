...
Last updated: 2026-08-25 14:10:00
...
- 已释放的`expression_single_mutant_selection_v3_20260825` 30条单突仍是不可改写的V3上游短名单；现行父单突专家审查/选择池在其后显式加入用户指定的稳定词探索突变`T99F`，共31条、23个位点。`T99F`保留上游`not_eligible`/`not_selected`状态，稳定词新增只是一项待实验检验的假设；当前尚未从31条中选择15条父单突，也未生成最终15条双突。
...
- `V3（现行）`：15条单突加15条双突。单突父本从当前31条审查/选择池中选择；该池由不可改写的30条V3上游短名单和用户补充的`T99F`组成。双突只从这15条父单突的完整有效无序配对空间中选择；后续双突须按完整双突序列重新计算NetSolP U、NetSolP S和NanoMelt预测Tm，AntiFold只沿用父单突的负向否决角色，不把单突logP相加成双突得分。
...
- 版本名指最终面板合同；`expression_single_mutant_selection_v3_20260825`是不可改写的30条上游筛选制品，31条专家审查/选择池是加入`T99F`后的下游制品，二者都不是现行最终面板。
...
11. 下一阶段才从31条审查池中选择15条不同位置优先的父单突，枚举其完整有效无序配对空间；若15条均位于不同位置，理论空间为`C(15,2)=105`条。重新计算完整双突序列的NetSolP U、NetSolP S和NanoMelt预测Tm后选择15条双突，形成15+15最终面板。
...
- V3专家审查覆盖31条/23个位点；27条实验复合物主证据、4条AF3-only低置信度证据。结构制品共63张图：31张主候选、7张AF3敏感性、23张WT位点近景和2张来源总览；评价CSV与视图均已回读，父单突选择保持`not_performed`。
