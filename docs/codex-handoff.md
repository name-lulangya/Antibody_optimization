...
Last updated: 2026-08-25 12:01:00
...
10. 下一阶段只从该30条短名单选择15条不同位置优先的父单突，枚举其完整有效无序配对空间；若15条均位于不同位置，理论空间为`C(15,2)=105`条。重新计算完整双突序列的NetSolP U、NetSolP S和NanoMelt预测Tm后选择15条双突，形成15+15最终面板。
11. V1旧30单突试选、V2的19条父单突、162条双突及19+11面板均只保留为历史计算provenance；不修改这些既有结果，也不把其候选、配额、分数或排名用于V3。
