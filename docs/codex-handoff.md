...
Last updated: 2026-08-15 16:21:34
...
11. **工具分工（当前选择）**：AntiFold评价候选对既定Nb252结构的条件相容性；NetSolP U/S仅作表达/溶解性辅助相容性信号；TNP只作developability风险工具；NanoMelt只提供相对WT的VHH热稳定性预测，不再承担yield排序。nanoBERT已退出，且不再部署同类型重复工具投票。PyRosetta复核完整亲和力组合，AF3只复核少量终选构象。
...
1. 建立AntiFold最小评分路线，在WT和现有亲和力核心上验证结构条件相容性；不启动81位framework采样，也不并行部署同类AbMPNN。
2. 排除`R45C/R45V`常规组合后设计少量机制互补CDR3亲和力双突；对WT、单突和完整组合统一计算亲和力、NetSolP U/S、NanoMelt相对WT预测Tm、TNP、AntiFold与化学风险，再按Pareto关系分层并形成最终30条及必要对照。NanoMelt与TNP均不提供yield排序。
