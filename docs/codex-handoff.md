...
Last updated: 2026-08-16 22:01:47
...
9. **完整双突合同与扫描（已完成；V2联合分析本地实现完成、远程待运行）**：86条完整128-aa双突已全部通过NetSolP、NanoMelt、TNP和3重复配对PyRosetta评分；PyRosetta完成258次突变体评估、覆盖45个位置对，实测12099.791012秒。扫描阶段未筛选。V1联合幅度分级为`28/12/2/44`，只是计算证据分层。V2已实现联接实验参考及配对WT的VHH接触/NK2R表位保持率、界面Cα RMSD和完整重复信息；结构门直接读取已释放校准阈值，不要求接触集合精确一致。远程V2尚未运行，因此当前还没有结构门后的真实类别计数。AntiFold仍只是两个固定骨架单点分数的数学加和，未评价双突上位性。NetSolP非加和残差支持使用完整双突重算值。
...
- 当前全套为`222 passed, 1 skipped, 4 subtests passed`，唯一skip为Windows真实symlink权限测试；V2专项6项通过。Python编译、V2 Slurm语法和`git diff --check`通过。V2绘图已用完整86行合成结构证据执行验证并修复分类标签拥挤；真实V2图要等远程复用原始摘要后生成。

## Required Next Steps

1. 提交并推送V2实现后，远程pull并运行`sbatch scripts/candidate_design/submit_double_mutant_analysis.slurm`。该作业只读取既有`results/candidate_design/double_mutant_scan_20260816/`，输出新的`double_mutant_scan_review_v2_20260816/`和轻量run summary；不重跑四个评分任务，预计数分钟。
2. pull回V2结果后核验86条结构状态、阻断原因、四项保持率、界面RMSD和新图，再形成亲和力驱动、性质驱动和组合驱动三类科学短名单。
3. 仅当近邻候选选择或机制解释确实需要时，对少量优先组合在统一位置对/可动集合/seed下补做`WT、single A、single B、double AB`四态PyRosetta分解；不对86条普遍重跑。随后仅让综合非劣且具有明确收益的少量组合进入AF3完整构象复核，最终形成30条待实验序列。
