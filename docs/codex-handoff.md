...
Last updated: 2026-08-23 15:17:20
...
- 最终向合作者提供30条Nb252候选：19条已批准单突和11条仅由这19条单突两两组合得到的双突。F30、Q1、T27各保留3条单突，其余10个位点各保留1条；同一位置的不同替换不得组合，不生成三突或更高阶突变。
...
7. 导师/用户已将现行面板组成改为19条单突加11条双突。现有30条单突试选及全部旧制品保持不变，仅作为新路线的冻结上游；独立下游已按F30/Q1/T27各3条、其余10个位点各1条确定19条父单突。精确序列、顺序和逐条理由只以`expression_single_mutant_parent19_20260822/expression_single_mutant_parent19_contract.json`为权威来源。
8. 19条父单突的完整组合空间已独立生成：171个无序对中9个为同位点互斥对，释放162条唯一不同位置双突和WT加162条共同评分样本。每条均在完整序列上重算风险与稳定词；19条双突出现稳定词净新增，其中18条含T99F，另1条T27F+F29T为组合后才形成的命中，组合后无既定硬序列风险。AntiFold保留两个组成位置的来源与幅度证据，102条同视图组合可记录固定骨架可加值，60条实验/AF3混合视图组合禁止跨视图相加；这两类均不表示双突上位性。
9. 162条双突的远程NetSolP/NanoMelt评分和结果联接已完成，矩阵门为pass。相对WT，U/S/Tm原始值为正的候选分别有93/125/106条；达到中等或明显有利档的分别有31/35/54条，中等或明显不利的分别有19/2/14条。组合预测不可直接按单突相加：U/S双突值与组成单突加和值的Spearman为0.579/0.660，而Tm为0.995；因此终选使用真实双突U/S/Tm结果，而不是父单突分数拼接。
10. 最终19+11面板已独立释放。162条中84条通过“无硬风险、U/S/Tm/AntiFold无中等或明显不利、至少一个家族中等或明显有利”门；A/B/C是证据广度层而非整层全收名单，双突名额固定为11。分类词典序优化在位置对最多1条、精确组成突变最多2次、reported位置最多3次的约束下，得到A层最大可行数3和在此前提下B层最大可行数4；A层5条中的3条F30×Q5V替换共享同一位置对，只能保留1条。其余4席由同样通过资格门的C层补足，使11条覆盖13个精确组成突变和10个reported位置。稳定词只作后置软偏好，本次11条中无稳定词净新增，但19条单突中的T99F仍保留。精确11条、73条替补、162行审计、最终30条CSV/FASTA和优化目标以`expression_final_19plus11_panel_20260822`机器制品为唯一权威来源。
...
- `expression_parent_19_single_mutant_release=pass`：现有30条试选未改写；19条单突父集及30→19审计已独立生成并通过3/3/3加10×1计数、序列身份和未来组合数核验。
- `double_mutant_enumeration_release=pass`：162条不同位置双突、9条同位点互斥审计、完整序列风险/稳定词证据及163条共同评分样本已经生成；其后性质矩阵与终选分别由下列独立阶段门记录。
- `expression_double_mutant_property_matrix_release=pass`：162条双突均有NetSolP U/S和NanoMelt预测Tm，组成位点AntiFold证据、完整序列风险、稳定词变化、幅度档和预测器非加和残差均已联接；该门不代表已选择候选。
- `final_19_single_plus_11_double_panel_release=pass`：19条已批准单突与11条从完整162行矩阵分类终选的双突已组成30条唯一128-aa候选；硬约束、多样性、序列身份和19/11计数均通过。该门表示可进入实验测试，不表示表达量已经实验验证。

## 本轮验证状态

- 19条父单突制品已回读：19条唯一128-aa序列覆盖13个位点，均保留末端SSGS，无硬序列风险或明显不利档；F30/Q1/T27各3条，其余10个位点各1条。
...
- 当前项目报告以连续的847→40→36→19单突筛选和171→162→84→11双突筛选叙述完整路线，不把中间集合写成独立实验阶段。双突A/B/C/无中等有利家族在162条总空间中分别为5/48/80/29条，资格门后为5/39/40/0条，最终选择为3/4/4/0条。唯一PPT保留原前10页，第11页仅更新结果表述，并追加3页介绍19条父单突、双突分层和最终30条面板；报告、演示稿及最终CSV/FASTA集中位于`docs/result_artifacts/weekly_report_result/report_2026_W34_nb252_expression_route/`。
