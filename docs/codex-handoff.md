...
Last updated: 2026-08-15 16:02:17
...
11. **工具分工（当前选择）**：AntiFold只评价亲和力候选对既定Nb252结构的条件相容性，不再负责大范围framework突变生成；nanoBERT已退出排序/过滤。NetSolP U/S仅作完整候选的辅助相容性证据；TNP验证已完成且只保留为developability风险工具。VHH专用Tm路线固定为NanoMelt 1.4.0，不再部署同类型工具重复投票：authoritative 128-aa Nb252 smoke中实际评分126-aa VHH域、裁去不可突变末端`GS`，预测表观Tm为65.18 °C；这只是模型输出，不是实验Tm。真实批量日志确认47条输入中43条通过NanoMelt ANARCI对齐、4条未返回；活动合同保留47条身份并以官方CSV的ID差集记录4条工具特异`nanomelt_not_scored`，不补写Tm，统计只用43条实际评分样本。若预声明统计门通过，弱排序release仅适用于NanoMelt可评分的标准VHH域候选，不能外推到4条未评分输入。远程ANARCI实际为Bioconda 2024.05.21且API/HMM完整，但残留pip 1.3 distribution metadata；评分入口按运行时代码/数据库核验并仅记录旧metadata作诊断。OpenMM要求`Reference/CPU`至少存在，计算节点额外出现的`OpenCL`等平台只记录、不阻断。PyRosetta复核完整亲和力组合，AF3只复核少量终选构象。
...
1. 远程重新执行`bash scripts/candidate_design/submit_nanomelt_yield_validation.sh`，同步43条真实NanoMelt评分和4条按ID差集确定的`nanomelt_not_scored`身份；按实际评分子集执行来源分层、90%序列簇外、LLJ有序/删失和Nb252影响分析。若全部统计门通过，只释放适用于NanoMelt可评分标准VHH域候选的弱排序证据；否则降为相容性过滤或不支持使用。不得猜测4条身份、补写Tm，或把关联解释为实验Tm、因果关系或mg/L预测。
