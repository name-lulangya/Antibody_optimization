...
Last updated: 2026-08-22 15:00:00
...
7. 导师/用户已将现行面板组成改为19条单突加11条双突。现有30条单突试选及全部旧制品保持不变，仅作为新路线的冻结上游；独立下游已按F30/Q1/T27各3条、其余10个位点各1条确定19条父单突。精确序列、顺序和逐条理由只以`expression_single_mutant_parent19_20260822/expression_single_mutant_parent19_contract.json`为权威来源。19条共有171个无序对，排除9个同位点互斥对后，下一阶段完整合法双突空间为162条；本阶段尚未枚举或评分双突。
...
- `double_mutant_enumeration_release=ready`：允许下一阶段从批准的19条父单突枚举162条不同位置双突；当前尚未执行枚举、性质评分或11条终选。
- `final_19_single_plus_11_double_panel_release=blocked`：等待162条双突的NetSolP/NanoMelt评价、AntiFold组成位点兼容性审查、完整双突序列风险复核和11条选择。

## 本轮验证状态

- 19条父单突制品已回读：19条唯一128-aa序列覆盖13个位点，均保留末端SSGS，无硬序列风险或明显不利档；F30/Q1/T27各3条，其余10个位点各1条，未来合法双突数严格为162。全套测试为269 passed、1 skipped、4 subtests passed。
