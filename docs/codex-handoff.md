...
Last updated: 2026-08-17 15:20:00
...
- Git：`main`，远程 `git@github.com:name-lulangya/Antibody_optimization.git`；V2.1结果分析提交`1308b7a`已推送，开始本轮实现时`HEAD=origin/main`。
...
14. **最终30条序列面板（已冻结）**：`final_candidate_panel_20260817/`以独立显式决策JSON联接原始36行能量终审模板，gate为`pass/ready_for_experimental_submission`。最终为6条亲和力单突、6条性质单突和18条组合突变；30条均为唯一128-aa序列，Cys22/Cys95、总Cys数和末端`SSGS`通过。排除高风险且无复合物改善的`D101W`、`I103W`和强烈分离态驱动的`Q1D;R45V`；保留`F30S;E105L`、`R45T;E105L`、`S55G;E105F`为3条储备，并将`A23S;R45V`、`F30T;E105F`、`R45T;E105F`由储备提升。双突任一组成突变最多出现5次。最终仍含9条分离态驱动候选，均按低置信性质/机制假设保留而非宣称亲和力改善；全部30条仅为计算优先、待实验验证。

## Verification

- 当前全套为`234 passed, 1 skipped, 4 subtests passed`，唯一skip仍为Windows真实symlink权限测试；`pip check`、Python编译和`git diff --check`通过。最终冻结gate通过30条选择、3条储备和3条排除；30个唯一序列、FASTA记录、128-aa长度、突变标签、Cys22/Cys95、总Cys数、末端`SSGS`及双突组成上限均经真实制品复核。最终PNG已人工检查，类别、能量来源和记录风险计数清晰。

## Required Next Steps

1. 将`final_candidates_30.csv`和`final_candidates_30.fasta`作为下一步实验交付序列源；WT Nb252作为30条之外的独立阳性/母本对照。不要从图、聊天或手抄序列重建订单。
2. 与合作者确认实验批量和检测资源后，设计BL21同体系表达、纯化、亲和力、热稳定/聚集及必要功能实验的板位、重复和预定义判定规则；优先保留单突—双突的可拆解关系，预测类别不替代随机化板位和WT重复。
3. 在真实实验数据返回前不追加批量AF3、PyRosetta或局部yield模型；仅在订单前发现具体序列/构建体问题，或实验后出现需要解释的矛盾结果时条件触发。
