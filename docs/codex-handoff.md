...
Last updated: 2026-08-25 12:30:00
...
- 已释放的`expression_single_mutant_selection_v3_20260825` 30条单突是V3选择15条父单突的只读上游短名单，不再直接等同于最终30条交付面板；当前尚未完成15条父单突、其完整有效无序双突空间及最终15条双突的选择。
...
- `V3（现行）`：15条单突加15条双突。单突父本只从当前V3 30条单突短名单中选择；双突只从这15条父单突的完整有效无序配对空间中选择。后续双突须按完整双突序列重新计算NetSolP U、NetSolP S和NanoMelt预测Tm；AntiFold只沿用父单突的负向否决角色，不把单突logP相加成双突得分。
...
- 版本名指最终面板合同；`expression_single_mutant_selection_v3_20260825`是V3使用的上游单突筛选制品。两者名称相近，但该30条单突制品不是现行最终面板。
...
6. V3把NetSolP U、NetSolP S和NanoMelt预测Tm作为三个独立正向指标，仍使用冻结幅度档且不以档内小数排序。候选至少有一个指标达到中等/明显改善，三个性质指标不得明显恶化；稳定词只作最后平局条件，不能单独获得名额。
...
10. 30条上游单突的结构/VHH专家审查已经完成：26条以实验复合物为主证据，L11Y、L11M、I28Y、F29Q因实验坐标缺失仅作AF3单体低置信度判断；30条均有独立ChimeraX单rotamer视图和逐条结构、溶解度、热稳定性解释。该审查明确未选择或淘汰父单突。
11. 下一阶段才从该30条短名单及专家审查中选择15条不同位置优先的父单突，枚举其完整有效无序配对空间；若15条均位于不同位置，理论空间为`C(15,2)=105`条。重新计算完整双突序列的NetSolP U、NetSolP S和NanoMelt预测Tm后选择15条双突，形成15+15最终面板。
12. V1旧30单突试选、V2的19条父单突、162条双突及19+11面板均只保留为历史计算provenance；不修改这些既有结果，也不把其候选、配额、分数或排名用于V3。
...
- `v3_parent_single_expert_review=pass`：30条均已完成来源分层的结构量化、独立单rotamer可视化和VHH专家判断；该门不执行父单突选择。
- `final_panel_v3_selection=not_started`：15条父单突、有效双突空间及最终15条双突尚未选择或释放。
...
- V3专家审查覆盖30条/23个位点；26条实验复合物主证据、4条AF3-only低置信度证据，另有6张缺口边界/CDR3 AF3敏感性图。所有候选均未命中冻结界面；7条位于冻结界面残基4.0埃壳层内、1条位于4.0–4.5埃边界壳层并已单独标记。30条逐候选rotamer图和评价CSV均已人工回读，父单突选择保持`not_performed`。
- 现有项目报告和PPT仍描述已退出的V2 19+11历史面板，尚未更新为V3 15+15合同，不得作为当前候选路线的状态来源。
