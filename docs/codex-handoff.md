...
Last updated: 2026-08-25 21:18:20
...
15. V3计算链与报告就绪性审计已经完成，机器检查18/18通过，未发现序列错配、冻结/界面违规或最终面板身份错误；结论为`pass_with_material_caveats`。V3项目报告DOCX/PDF已从终选manifest和最终30条CSV/FASTA独立生成，19页渲染逐页通过；审计、报告、manifest和报告专用图均位于`docs/result_artifacts/weekly_report_result/Nb252_V3_expression_report/`。
16. 当前为report-only交付：历史19＋11的V2报告保持原样，V3 PPT和压缩包未被本轮要求且明确不在范围内。后续若进入实验准备，应以V3报告和最终15＋15机器制品为准，并按报告中的WT配对、重复、产量、质量与功能保持读出执行；若另行要求PPT或交付包，再从同一V3权威来源生成。
...
- `final_panel_v3_release=pass`：最终15条单突加15条双突的CSV/FASTA、102行决策审计、manifest和图已释放；V3项目报告同步为`pass`，PPT/压缩包为`out_of_scope_not_requested`。
- `v3_report_audit=pass_with_material_caveats`：18/18机器完整性检查与report-only完成门通过；V3 DOCX/PDF已绑定manifest并可正式使用。4个远程raw评分文件未本地归档，且T99F、AF3-only、软责任与未做双突侧链建模等边界已在报告披露。
...
- 历史V2 19+11项目报告保持原样；新的V3 15+15 DOCX/PDF已在独立V3报告目录中生成。正文和图注明确AntiFold仅用于风险排除，不提议、不奖励、不排序候选，双突不计算联合AntiFold分数。
- V3审计独立回接847→30→31→15→102→15→最终30，确认CSV/FASTA、突变重建、硬约束和本地manifest哈希绑定均一致；项目报告已纳入远程raw缺档、T99F/A23R、AF3-only、软责任、F30缺口边界和双突未建模等限制。DOCX/PDF完成19页逐页渲染验收，PPT与压缩包不在本轮范围。
