...
Last updated: 2026-08-25 15:55:38
...
- 已释放的`expression_single_mutant_selection_v3_20260825` 30条单突仍是不可改写的V3上游短名单；其后加入用户指定的`T99F`形成31条专家审查池。`v3_parent_single_selection_20260825`已冻结15条父单突及31条逐条选择/淘汰理由：9条具有至少一项强改善，5条因高置信明确物理风险硬排，11条因同位点竞争或综合证据较弱未入选。`T99F`保留上游`not_eligible`/`not_selected`状态，仅作为用户指定的稳定词探索例外入选；最终15条双突尚未生成。
...
- `V3（现行）`：15条单突加15条双突。15条父单突已从31条专家审查池中冻结，覆盖12个位点；L11、F30和K75各保留两个替换，三对同位点组合无效，因此105个理论无序对中有102个有效双突。后续须对完整双突序列重新计算NetSolP U、NetSolP S和NanoMelt预测Tm；AntiFold只沿用负向否决角色，不把单突logP相加成双突得分。
...
- 版本名指最终面板合同；`expression_single_mutant_selection_v3_20260825`是不可改写的30条上游筛选制品，31条专家审查制品保存结构/VHH判断，`v3_parent_single_selection_20260825`是15条父单突及全部31条决策理由的现行权威来源；这些中间制品均不等同于最终30条面板。
...
11. 15条父单突已经确定：其中9条至少有一个U/S/Tm强改善档、5条为中等性质证据补充、`T99F`为唯一稳定词探索例外。专家意见只对5条高置信明确物理风险执行硬排；其余中低置信担忧仅作标注或降级。下一阶段枚举102条有效无序双突，按完整双突序列重新计算NetSolP U、NetSolP S和NanoMelt预测Tm，再选择15条双突形成15+15最终面板。
...
- `final_panel_v3_double_selection=not_started`：102条有效双突尚未生成、重新评分或筛选，最终15条双突尚未释放。
