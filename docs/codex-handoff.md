...
Last updated: 2026-08-25 19:03:05
...
- `V3（现行）`：15条单突加15条双突。15条父单突已从31条专家审查池中冻结，覆盖12个位点；L11、F30和K75各保留两个替换，三对同位点组合无效，因此105个理论无序对中有102个有效双突。102条完整128-aa双突已完成NetSolP U/S和NanoMelt预测Tm实算且未作预筛；下一步从完整矩阵执行幅度档、责任基序和结构/VHH专家复核，再显式选择15条。AntiFold只保留两个组成单突各自的负向否决证据，不相加、不生成双突得分。
...
12. 15条父单突的105个理论无序对已完整审计；排除`L11Y+L11M`、`F30S+F30N`和`K75A+K75E`三个同位点互斥对后，102条双突已完成NetSolP/NanoMelt共同评分。91条至少一项U/S/Tm达到中等或强改善，84条有性质支持且无中/强不利，6条含中等权衡，1条含强不利，11条没有中/强正向；8条三个指标均达到中等或强改善。最终15条仍未选择。
13. 下一步先按冻结幅度档形成性质证据层，重点复核8条三指标支持、7条中/强不利权衡及6条直接局部邻近组合；同时保留父单突专家意见、AntiFold组成单突否决证据和稳定词探索语义。`N76G+F30N`必须按位置集合标注F30N新增的reported 30 `NG`脱酰胺基序，不能被N76G删除reported 76 `NT`后的净计数0掩盖。完成逐条专家审计后再显式冻结15条双突和最终30条面板。
...
- `final_panel_v3_double_selection=not_started`：完整性质矩阵已可用，但尚未完成逐条结构/VHH复核或最终15条选择。
...
- V3双突结果已回读为102条唯一128-aa双突；两工具均对WT加102条共同表103/103通过，NanoMelt全部评分前126 aa并仅裁末端`GS`。AntiFold仍仅保留组成单突证据；模型非加和残差明确不是物理上位性。远程原始`results/`和`logs/`受Git忽略，当前本地可独立核验的是其SHA绑定的正式矩阵、manifest与run summary。同步审计另记录一条位置级软风险勘误并修复后续代码语义；最终15条没有自动生成。
