...
Last updated: 2026-08-12 15:00:05
...
9. **亲和力严格复核（已实现、待远程运行）**：合作者最终只需30条优化序列，因此生产范围修订为全部Tier 1/2共48候选，加补充独有位置的Tier 3 `D33N`和`Y115F`，共50候选×20样本=1000任务；其余37条Tier 3不进入本轮。任务身份和seed不随调度并发变化。默认数组并发12、P90投影约24.30小时；支持以`--concurrency N`提交/续跑，以及对已提交数组动态修改节流。任务级续传只复用六文件完整、身份一致且结构安全pass的结果，部分或冲突结果阻断且不覆盖。日志统一写入`logs/flex_ddg_production/`；全部任务完成后先生成未筛选ensemble摘要，再单独设计统一筛选，当前仍不组合突变。
...
1. 远程pull后运行`bash scripts/candidate_design/submit_flex_ddg_production.sh`；默认并发12。若需调整，提交或续跑时传`--concurrency N`；队列中按包装输出的命令修改本次所有数组节流。
2. 监控`logs/flex_ddg_production/`及本次`resume_state.json`；中断后先确保旧任务不再运行，再重跑包装。完整任务会自动跳过，部分/冲突任务需先分析，不会被覆盖。
3. 1000任务及自动汇总完成后pull结果，独立核验完整性、20样本分布、方向支持和接触保持，再设计一次统一ensemble筛选并形成机制多样的亲和力单点实验面板；最终30条还需为表达量/稳定性及少量组合设计留出空间，仍不把REU转换为实验亲和力。
