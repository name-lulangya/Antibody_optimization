...
Last updated: 2026-08-16 23:07:24
...
9. **完整双突合同与扫描（已完成；V2结果已同步但门语义需校正）**：86条均完成四工具评分，V2实测2.297871秒并成功联接结构字段。原V2输出为结构pass/blocked=`29/57`、最终类别`balanced/structure-review/tradeoff=8/57/21`；但57条全部只因相对实验结构的NK2R表位保持率为`33/37=0.891892`，略低于通用0.90阈值。它们相对各自位置对配对WT的NK2R保持率最低仍为`0.972973`，VHH最低`0.958333`，全体最大界面Cα RMSD仅`0.047716 Å`；57条中48条相对配对WT的VHH和NK2R接触均完全保持。故57代表统一的实验参考/局部准备敏感性，不是57条结构崩坏或淘汰结论。V2原始字段保留，但其绝对实验保持率不应继续作为硬门；应以配对WT和RMSD作主安全门，并单独审阅12条存在任一配对接触变化的候选。AntiFold仍只是两个固定骨架单点分数的数学加和，未评价双突上位性。
...
- 最近一次实现验证为`222 passed, 1 skipped, 4 subtests passed`，唯一skip为Windows真实symlink权限测试；V2专项6项通过。真实V2 gate、86行CSV和run summary已读回，图已人工检查，三面板、短标签、图例和说明无遮挡。当前问题是结构门科学语义过度保守，不是运行或数据完整性失败。

## Required Next Steps

1. 实现一次轻量V2.1科学语义修正：运行完整性、配对WT VHH/NK2R保持率和界面RMSD作为结构安全门；相对实验结构的低于0.90保持率改为准备敏感性标记，不再覆盖原多目标类别。同步从既有逐重复接触集合导出12条发生任一配对接触变化候选的具体lost/gained残基；不重跑模型。
2. pull回V2.1结果后形成亲和力驱动、性质驱动和组合驱动三类科学短名单。当前V1的`28/12/2/44`可作门前参考，但需等V2.1接触残基核验后再决定哪些进入AF3。
