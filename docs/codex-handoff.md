...
Last updated: 2026-08-16 17:15:03
...
10. **统一安全门与后续组合（当前有效）**：80条V2证据经非冗余结构复核和综合风险缩减后，当前活跃单突为14条：5条组合模块审阅`Q1D/A23S/F30A/F30S/F30T`、8条单突测试`R45V/D101W/I103W/E105F/E105L/N107A/S114M/S55G`及亲和力备选`R45T`。F30三替换同位点互斥；`combination_ready`只表示可进入组合合同，不证明双突相容。后续按亲和力×亲和力、性质×性质和亲和力×性质生成小型、可解释的双突假设并逐条重算，不恢复本轮已降级的高风险性质模块。
...
- 阶段0专项：`3 passed`；统一单突专项`4 passed`；统一性质评分与既有NetSolP/NanoMelt回归共`16 passed`；统一TNP专项`4 passed`；性质候选PyRosetta科学review专项`6 passed`；统一单突安全门专项`2 passed`；非冗余定点结构复核专项`5 passed`；单突短名单专项`2 passed`。当前全套为`216 passed, 1 skipped, 4 subtests passed`；真实远程runtime和V2 analysis gate均为`pass`，定点结构图与单突短名单图均已人工检查无遮挡。
...
1. 从14条活跃单突建立首轮双突设计合同：性质模块仅使用`Q1D/A23S/F30A/F30S/F30T`，`S55G`保持单突测试；亲和力模块先复核`R45T`及7条亲和力核心的组合适用性。排除同位点互斥和明显结构/化学冲突，不恢复16条已降级性质候选，也不为凑30条降低门槛。
