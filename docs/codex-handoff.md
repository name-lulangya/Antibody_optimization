...
Last updated: 2026-08-22 18:00:00
...
8. 19条父单突的完整组合空间已独立生成：171个无序对中9个为同位点互斥对，释放162条唯一不同位置双突和WT加162条共同评分样本。每条均在完整序列上重算风险与稳定词；19条双突出现稳定词净新增，其中18条含T99F，另1条T27F+F29T为组合后才形成的命中，组合后无既定硬序列风险。AntiFold保留两个组成位置的来源与幅度证据，102条同视图组合可记录固定骨架可加值，60条实验/AF3混合视图组合禁止跨视图相加；这两类均不表示双突上位性。下一步只需远程运行NetSolP和NanoMelt后合并162行矩阵，再按幅度、风险、位点和组成多样性选择11条。
...
- `expression_double_mutant_property_matrix_release=ready_to_run_remote`：执行`bash scripts/candidate_design/run_expression_double_mutant_scan.sh`，单个Slurm作业顺序运行NetSolP和NanoMelt并合并矩阵；预计低于2小时，使用`batch`、1 GPU和12 CPU，日志进入`logs/expression_double_mutant_scan/`。
- `final_19_single_plus_11_double_panel_release=blocked`：等待162条双突性质矩阵完成及随后11条双突选择。

## 本轮验证状态

- 19条父单突制品已回读：19条唯一128-aa序列覆盖13个位点，均保留末端SSGS，无硬序列风险或明显不利档；F30/Q1/T27各3条，其余10个位点各1条。
- 162条双突计划已回读：候选ID和序列均唯一，每条恰好改变两个不同reported位置，保留末端SSGS和原有Cys且不新增Cys；9条同位点互斥组合单列，163条NetSolP/NanoMelt输入完全共用同一顺序。远程评分尚未运行。
