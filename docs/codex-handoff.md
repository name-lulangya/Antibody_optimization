...
Last updated: 2026-08-25 18:30:00
...
- 已释放的`expression_single_mutant_selection_v3_20260825` 30条单突仍是不可改写的V3上游短名单；其后加入用户指定的`T99F`形成31条专家审查池。`v3_parent_single_selection_20260825`已冻结15条父单突及31条逐条选择/淘汰理由：9条具有至少一项强改善，5条因高置信明确物理风险硬排，11条因同位点竞争或综合证据较弱未入选。`T99F`保留上游`not_eligible`/`not_selected`状态，仅作为用户指定的稳定词探索例外入选。基于这15条父单突的102条有效完整双突现已全部枚举并冻结为共同评分输入；远程NetSolP/NanoMelt尚未启动，最终15条双突尚未选择。
...
- `V3（现行）`：15条单突加15条双突。15条父单突已从31条专家审查池中冻结，覆盖12个位点；L11、F30和K75各保留两个替换，三对同位点组合无效，因此105个理论无序对中有102个有效双突。102条完整128-aa双突已全部枚举且未作性质预筛；下一步须运行远程NetSolP U/S和NanoMelt预测Tm，完成102行性质矩阵后再选择15条。AntiFold只保留两个组成单突各自的负向否决证据，不相加、不生成双突得分。
...
12. 15条父单突的105个理论无序对已完整枚举；排除`L11Y+L11M`、`F30S+F30N`和`K75A+K75E`三个同位点互斥对后释放102条双突及WT加102条共同评分表。每条双突均按完整序列重算风险和稳定词，并保留实验/AF3来源分列的WT位点结构初筛；当前没有运行远程预测，也没有选择最终15条。
13. 下一步在远程仓库根目录执行`bash scripts/candidate_design/run_v3_double_mutant_scan.sh`。单个非数组Slurm作业将顺序运行NetSolP、NanoMelt和项目环境联接，要求两工具对同一103条ID/序列完整覆盖；完成后才进入幅度档审核、结构专家复核和最终15条双突选择。
...
- `v3_double_mutant_property_matrix=not_started`：远程NetSolP/NanoMelt尚未启动，102条双突尚无完整实算性质矩阵。
- `final_panel_v3_double_selection=not_started`：102条有效双突虽已生成，但尚未完成远程性质评分、后续复核或最终15条选择。
...
- V3双突计划已回读为102条唯一128-aa双突和WT加102条共同评分样本；全部恰有两个不同reported位置替换，保留末端SSGS、原有两枚Cys且无硬序列风险。稳定词净新增15条、无减少项；64条位置对可用实验复合物WT坐标作距离初筛，38条因至少一处实验缺失仅保留独立AF3预测距离来源。该结构初筛不代表突变后结构效应；专项测试9 passed，全套验证309 passed、1 skipped、4 subtests passed，远程U/S/Tm仍未运行。
