...
- **行动**：报告准确称为“规则化专家复核与WT几何分层”，将15条双突的性质定位为实验优先级证据。
...
- **行动**：独立审计保留AF3-only与F30 gap-boundary证据；项目报告按当前展示口径不再逐项展开未触发明确排除规则的候选级结构推测。
...
- **行动**：上述事实继续保留在独立审计记录中；项目报告说明明确高风险突变已在专家排除步骤移除，不再把未触发明确排除规则的候选级推测写入入选表。
...
- **行动**：V3报告不直接复用上游单突“Final 30”整图；报告采用V3专用拆图，并在图注中说明U/S是分别评价的同模型输出，不再在入选候选图中叠加结构或软风险注记。
...
2. 独立审计保留T99F、A23R、F30缺口边界、AF3-only、F30N脱酰胺和L11M氧化等证据；项目报告不再展开候选级推测性风险。
