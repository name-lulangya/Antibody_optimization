...
## Degenerate stable-word evaluation

- `antibody_optimization.stable_words`
  - 用途：按固定12符号简并字母表编码标准氨基酸序列，执行大小写敏感的精确连续子串扫描，保留重叠和嵌套命中，并比较单突相对WT新增或减少的稳定词出现位置。
  - 输入与返回：接收一行一词的稳定词库、权威亲本和已验证单突；返回逐序列描述符、逐候选汇总及带reported 1-based起止位置的变化长表。47条产量验证复用现有来源分层连续统计和训练折内阈值的嵌套分类函数。
  - 算法假设：`h`与`H`严格不同；出现的身份键为`stable_word + start + end`；同一稳定词的重叠出现分别计数。稳定词增加仅是外部序列模式先验，不是实测稳定性、表达量或因果证据。
  - 明确不支持：模糊匹配、自动修复非法字符、用稳定词作硬过滤、训练高容量产量模型，或自动选择最终30条单突。
- `scripts/candidate_design/build_stable_word_single_mutant_features.py`
  - 用途：把冻结稳定词库应用于完整847条表达性质矩阵，写出规范词库、逐候选新增/减少汇总、变化长表、增强矩阵、合同、gate和run summary；默认拒绝覆盖。
...
- `antibody_optimization.v3_expert_review_pool`
  - 用途：在不改写V3原始30条上游短名单的前提下，将用户指定的稳定词探索候选`T99F`确定性追加为第31条审查对象，并保留两类候选的来源角色和原始选择状态。
  - 输入与返回：接收原始30条短名单、847条V3审计和显式补充候选ID；返回连续编号的31条专家审查/父单突选择池。该模块只管理身份与顺序，不改变上游资格、评分或选择结论。
- `antibody_optimization.vhh_expert_review`与`antibody_optimization.vhh_expert_review_assessments`
  - 用途：为V3的31条父单突审查池生成不参与排序的结构/VHH专家审查。实验NK2R–Nb252复合物有坐标时为主证据；实验缺失位置仅用独立AF3 VHH上下文并固定低置信度。
  - 输入与返回：接收31条审查池、reported↔结构映射、实验/AF3坐标、FR对齐变换和冻结界面集合；返回WT相对SASA、主链几何、局部邻居、受体/界面壳层距离、模型敏感性、突变化学变化，以及逐候选结构、溶解度和热稳定性专家判断。
...
- `scripts/candidate_design/render_v3_parent_single_structure_review_chimerax.py`
  - 用途：在ChimeraX 1.12空白会话中为31条候选分别从原始来源坐标重新打开模型、执行单点Dunbrack `swapaa`并保存局部PNG；实验缺失位置用AF3，缺口边界和CDR3另存AF3敏感性视图。当前制品包含31张主候选、7张AF3敏感性、23张WT位点近景和2张来源总览，共63张图。
...
- `antibody_optimization.v3_double_mutants`
  ...
  - 算法假设：只组合不同reported位置且不作评分前性质筛选；AntiFold只保留两个组成单突各自的来源、ΔlogP、位置内排名和否决状态，不相加也不生成双突AntiFold分数。结构初筛只描述WT位点几何，AF3距离保持预测来源标签；U/S/Tm非加和残差只描述模型输出，不能称为物理上位性。序列责任基序同时保留净计数变化和按起始位置判断的新增标记，避免“删除一处、另处新增”被净计数抵消。
...
- `antibody_optimization.expression_parent_panel`
  ...
  - 算法假设：模块只执行显式决策和结构性核验，不重新解释小数分数、不运行预测器、不修改上游试选；选择理由由调用方提供并保留。
  - 明确不支持：从847条空间重新选择、自动推断“最佳”替换、枚举或评分双突、生成三突/更高阶突变，或把19条计算父集称为实测高表达序列。
- `scripts/candidate_design/select_expression_single_mutant_parent19.py`
  - 用途：固化当前F30/Q1/T27各3条及其余10个位点各1条的具体导师/用户决策，写出独立的19条CSV/FASTA、30行审计、合同、gate和run summary；现有30条制品仅作为只读输入。
