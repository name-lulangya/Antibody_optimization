...
def render_tnp_yield_figure(sample_rows: Sequence[Mapping[str, object]], metric_rows: Sequence[Mapping[str, object]], cv_rows: Sequence[Mapping[str, object]], *, png_path: Path, svg_path: Path) -> None:
    ...
    axes[0, 0].set(xlabel="TNP PSH (Kyte–Doolittle scale)", ylabel="Reported yield (source numeric value)", title="A  PSH and individual numeric observations")
    ...
    axes[0, 1].set(xlabel="Within-provider stratified Spearman ρ", xlim=(-1, 1), title="B  Six exploratory TNP associations")
    ...
    axes[1, 1].set(xlabel="LLJ reported yield group", ylabel="TNP PSH (Kyte–Doolittle scale)", title="D  LLJ ordinal/censored context")
