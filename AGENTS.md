...
- The active 30-sequence experimental panel must contain 19 approved single mutants and 11 double mutants enumerated only from those 19 single-mutant parents. At reported positions F30, Q1, and T27 retain three single substitutions each; at each of the other ten positions represented in the frozen 30-single-mutant trial panel retain one. Same-position alternative substitutions are mutually exclusive and must never be combined.
...
- The current deliverable is explicitly limited to 19 approved single substitutions plus 11 double substitutions selected from the complete valid pair space of those 19 parents. Do not introduce a double-mutant component outside the approved 19, do not generate triple/higher-order mutants, and do not use the superseded 30-single-only restriction as an active gate.
