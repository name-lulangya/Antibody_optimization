...
- Conserved positions identified by the approved multiple-sequence-alignment workflow must also be excluded from mutation generation. The alignment input scope, sequence preprocessing, numbering map, conservation metric and cutoff must be frozen in a machine-readable artifact before candidate generation.
...
- Before candidate generation, align the approved sequence set, map alignment columns reversibly to the 128-aa Nb252 reported-sequence indices, and freeze the conserved positions and conservation rule. Do not infer conservation from a raw column frequency without accounting for sequence scope, gaps, incomplete domains and duplicated or highly related sequences.
...
- Exclude mutations at the interface, frozen conserved positions, disulfide cysteines and terminal `SSGS`. Flag other mutations that may affect framework packing, charge patches, aggregation propensity, chemical liabilities, protease sensitivity, glycosylation motifs, or expression, and document any additional filtering rule and rationale.
