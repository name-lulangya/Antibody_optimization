...
This project optimizes the experimentally identified NK2R nanobody Nb252 for BL21 expression yield. The active design route no longer explicitly optimizes binding affinity; it preserves the experimentally defined interface and other protected sequence features while prioritizing expression-related predictions, scientific interpretability, and experimental testability.
...
- The collaborator-confirmed Nb252 design parent is the complete 128-aa reported sequence. Its terminal `SSGS` is not a linker, is part of the authoritative construct, and is immutable during candidate design: do not substitute or delete reported sequence positions 125–128. The terminal `GS` remains outside the provisional ANARCII/IMGT numbered span, so these four protected residues must be referenced by reported-sequence indices rather than invented IMGT positions.
- The experimentally defined Nb252 interface is now an immutable design region: every residue in the released 24-residue interface set must be excluded from mutation generation, regardless of any earlier affinity or property score.
- A position may be frozen specifically for natural conservation only when the approved multiple-sequence-alignment workflow passes its conservation gates and the Nb252 parent residue equals the agreed global/local natural dominant residue. If a position passes the same conservation gates but Nb252 differs from that dominant residue, do not freeze the parental residue as naturally conserved and do not open an unrestricted scan: allow only the single parent-to-consensus reversion, unless another immutable rule freezes the position. The alignment input scope, sequence preprocessing, numbering map, conservation metric, cutoff and resulting mutation policy must be frozen in a machine-readable artifact before candidate generation.
- The active final-panel contract is version V3: the released 30-sequence experimental panel contains 15 single mutants and 15 double mutants. The 15 double mutants were formed only from the 15 selected single-mutant parents; same-position alternatives are mutually exclusive and must never be combined.
- The released `expression_single_mutant_selection_v3_20260825` 30-single-mutant artifact remains the immutable upstream V3 shortlist and must not be rewritten. The parent-single expert-review pool is those 30 candidates plus the user-added stable-word exploratory mutant `T99F`, for 31 candidates across 23 reported positions. The active `v3_parent_single_selection_20260825` artifact freezes the approved 15 parents and the detailed disposition of all 31 reviewed candidates; it is the only authority for downstream V3 double-mutant generation. `T99F` retains its upstream `not_eligible`/`not_selected` status because it has no moderate or strong positive NetSolP U/S or NanoMelt signal, and enters the approved parent set only as an explicit user-directed stable-word exploration exception. Neither the immutable 30-row shortlist nor the 31-row review artifact is itself the final 30-sequence panel.
- The active final release is `docs/result_artifacts/candidate_design/v3_final_15plus15_panel_20260825/`; its manifest is the authority for the 102-row double-mutant review, selected 15 doubles and final 15-single plus 15-double panel. All 102 valid doubles were reviewed under the same rules. The 58 `enhanced` and 44 `standard` records differ only in review depth, not eligibility or rank. `T99F`-containing doubles have no mutation-specific trigger, quota, bonus, veto or selection exception.
- The current V3 project report and its completed readiness audit are colocated under `docs/result_artifacts/weekly_report_result/Nb252_V3_expression_report/`. The historical V2 report remains byte-for-byte unchanged. A V3 presentation and archive are outside the current report-only scope unless the user requests them later.
- Final-panel versions V1 and V2 are historical, read-only routes. V1 is the former 30-single-mutant trial route; V2 is the former 19-single-plus-11-double route with fixed F30/Q1/T27 quotas. Do not modify their code or result artifacts, and do not use their candidate lists, scores, pair results, quotas, or rankings to generate or rank the active V3 panel.
...
- `requirements-local.txt` is the tracked direct-dependency specification for that Windows environment, and `requirements-local.lock.txt` is its validated full pip snapshot. Keep both current when local project packages change; preserve the CPU-only PyTorch source unless the user explicitly establishes a different local compute route.
...
- PyRosetta at `/data/software/env/luly25/multi_ligand` and nanoBERT at `/data/software/env/luly25/vhh-lm` are historical tool-specific environments, not the project environment. The active expression-only route must not run PyRosetta/Rosetta, Flex ddG, nanoBERT, or use their scores to generate or rank candidates. Preserve their existing outputs only as historical provenance unless the user explicitly establishes a new project route.
- AntiFold is likewise a separate remote tool environment. Use only the environment, model location, loader workaround, single-process CUDA setting, and versions frozen in the current Git-tracked AntiFold environment contract; do not infer them from the source checkout or substitute the project environment.
- The active candidate-evaluation tool set is limited to NetSolP, NanoMelt, and AntiFold. Do not add back TNP, AF3 batch prediction, Rosetta-family scoring, nanoBERT, or redundant tools by default.
- AntiFold is a negative risk-exclusion signal only: it must never propose, reward, rank or break ties among candidates. The single-mutant exclusion requires both the frozen `delta logP <= -3` condition and a bottom-4-of-20 within-position state rank. Double mutants do not receive an AntiFold score; they only inherit the requirement that both constituent single mutants pass the negative exclusion rule.
- One or two new expression-related predictors may be evaluated sequentially against the 47-sequence yield dataset. A new predictor may enter candidate ranking only after its fixed output semantics, applicability, coverage and cross-validated relationship to yield have been documented; do not retain multiple tools of the same type merely for voting.
- Candidate generation must exclude the released experimental interface set, the frozen MSA-conserved set, Cys22/Cys95, reported positions 125–128 (`SSGS`), and any other machine-readable immutable positions before any property scoring occurs.
- The current V3 deliverable is the released set of 15 approved single substitutions plus 15 selected double substitutions in `v3_final_15plus15_panel_20260825`. Do not substitute a different double-mutant component, introduce a component outside the approved 15 parents, combine two alternatives at the same reported position, or generate triple/higher-order mutants.
- Preserve all V1/V2 scripts and result artifacts unchanged as historical provenance. They must remain excluded from active V3 generation, scoring, ranking, release gates, reports, and deliverable FASTA/CSV files unless the user explicitly requests a historical comparison.
...
- Give each script one primary workflow responsibility, such as structure preparation, sequence/numbering reconciliation, interface analysis, candidate generation, property prediction, expression-oriented candidate ranking, statistical analysis, or artifact rendering. Split independent stages instead of concentrating computation, plotting, reporting, and unrelated design routes in one entry point.
...
- In the `NK2R-252` model, the 24 collaborator-orange VHH residues have been remapped and exactly reproduced by the project definition: positive-occupancy polymer heavy-atom center distance strictly `<4.0 Å`, with H/D, water, ligand, glycan, ion, crystal and NCS copies excluded and altloc compatibility enforced. This reproducible set is now a hard mutation exclusion set. It remains an interface annotation rather than an energy-hotspot claim, but no active candidate may mutate any of these residues.
...
- Candidate mutation generation must preserve a traceable path from structural or sequence rationale to the exact candidate sequence. Record parental sequence, the single substitution, numbering scheme, MSA/conservation evidence, design method/version, constraints, and filtering decisions.
- Before candidate generation, align the approved sequence set and map alignment columns reversibly to the 128-aa Nb252 reported-sequence indices. Freeze a naturally conserved position only when Nb252 itself carries the agreed global/local dominant residue; for a highly conserved nonconsensus Nb252 position, permit only the shared natural-dominant reversion unless another hard constraint freezes it. Freeze the conservation rule and mutation policy. Do not infer conservation from a raw column frequency without accounting for sequence scope, gaps, incomplete domains and duplicated or highly related sequences.
...
- Exclude mutations at the interface, naturally conserved positions whose Nb252 residue matches the dominant residue, disulfide cysteines and terminal `SSGS`. At a highly conserved nonconsensus position, allow only the documented consensus reversion and still apply every overlapping hard exclusion. Flag other mutations that may affect framework packing, charge patches, aggregation propensity, chemical liabilities, protease sensitivity, glycosylation motifs, or expression, and document any additional filtering rule and rationale.
...
### Expression optimization and experimental validation

- The active optimization target is BL21 expression yield. NetSolP, NanoMelt and AntiFold remain distinct predictive signals rather than measured yield; stability or structural compatibility may be used as supporting constraints, but the workflow must not claim explicit affinity optimization.
...
- The collaborator has explicitly confirmed that the reported yield observations from LTT, WCC, and LLJ are directly comparable. Cross-source analysis is therefore allowed only while preserving the recorded observation semantics: LTT/WCC individual approximate values remain numeric, whereas LLJ group anchors and lower bounds remain group-level/censored rather than invented individual point estimates. This confirmation does not itself validate prediction accuracy for unseen Nb252 mutants.
- Do not use historical Rosetta affinity or energy scores in the active candidate ranking. If affinity is measured later, report it only as an experimental secondary outcome and do not retroactively describe the current design as affinity optimization.
- Candidate rankings must record predictor definitions, direction, transformations, missing-value handling, hard exclusions, any weights or Pareto method, tie-breaking, and software/version. Never infer or reuse a universal weighting formula.
- Tool validation against the 47-sequence yield dataset must include both continuous association and a prespecified classification analysis. Discretize yield and each predictor under explicit, reproducible rules; report ROC-AUC, precision-recall AUC, MCC, balanced accuracy, sensitivity, specificity and the selected decision threshold with uncertainty where feasible.
- Threshold selection and performance estimation must be separated by nested cross-validation, leave-one-out/leave-cluster-out resampling, or another documented out-of-sample procedure. Do not select a cutoff and report its performance on the same observations as if it were externally validated; preserve LLJ group-level/censored semantics instead of inventing individual yields.
...
- In V3 parent-single selection, use expert assessment as a hard exclusion only for a concrete physical risk supported by experimental coordinates and a high-confidence `structurally_concerning` judgment. Medium- or low-confidence concerns remain annotations or priority downgrades rather than automatic vetoes. Prioritize candidates with strong NetSolP U, NetSolP S, or NanoMelt magnitude-band improvements; preserve any explicitly approved hypothesis-only exception without relabeling it as predictor-qualified.
...
- Write Slurm standard-output and standard-error logs under a workflow-specific subdirectory of the project-level `logs/` directory, for example `logs/flex_ddg_production/`. Submission wrappers must create that subdirectory before calling `sbatch`; runtime logs are ignored operational files, not scientific result artifacts.
...
- Before starting a project-report task, verify that the corresponding audit report is stored beside that report under `docs/result_artifacts/weekly_report_result/<report-route>/` and is marked complete.
...
- Store each completed audit in the same versioned project-report directory as the report it governs, under `docs/result_artifacts/weekly_report_result/<report-route>/`. Use a descriptive versioned filename such as `<project>_V3_audit_report.md`, not an ISO-week-only filename. Record the audited date range and commit range, inputs inspected, checks performed, findings by risk, unverified items, unresolved issues, and whether the corresponding project report may be finalized.
- Unresolved `Critical` or `High` findings block the corresponding project-report finalization until they are corrected and rechecked. Lower-risk limitations must be disclosed when they affect report interpretation.
