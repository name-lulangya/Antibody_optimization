...
def _make_figures(out: Path, parents: list[dict[str, str]], doubles: list[dict[str, str]], gate: dict) -> dict[str, Path]:
    plt.rcParams.update({"font.family": [FONT, "Arial", "DejaVu Sans"], "axes.unicode_minus": False})
    fig_dir = out / "figures"
    fig_dir.mkdir(parents=True)
    sources = {
        "constraints": CURRENT_REPORT_DIR / "figures/source_constraint_tracks.png",
        "validation": CURRENT_REPORT_DIR / "figures/figure_tool_yield_validation.png",
    ...
    ax.set_ylim(0, 195)
    ax.spines[["top", "right"]].set_visible(False)
    ax.text(2.55, 78, "A层 3条\nB层 4条\nC层 4条", ha="center", va="center", fontsize=10,
            bbox={"boxstyle": "round,pad=0.4", "facecolor": "white", "edgecolor": "#777777"})
    fig.tight_layout()
...
def _add_footer(doc: Document) -> None:
    for section in doc.sections:
        ...
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        _font(p.add_run("Nb252 BL21表达量优化阶段报告"), size=8, color=GRAY)


def _build_report(out: Path, parents: list[dict[str, str]], doubles: list[dict[str, str]], panel: list[dict[str, str]], gate: dict, figures: dict[str, Path]) -> Path:
    doc = Document()
    _style_document(doc)
    p = doc.add_paragraph(style="Title"); p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    _font(p.add_run("Nb252纳米抗体BL21表达量优化阶段报告"), size=21, bold=True)
    ...
    _point(doc, "原30条单突试选进一步收缩为19条父单突：F30、Q1、T27各3条，其余10个位点各1条。")
    ...
    _page(doc)
    doc.add_heading("1. 设计目标与不可变约束", level=1)
    ...
    _page(doc)
    doc.add_heading("2. 天然VHH保守性参照", level=1)
    ...
    _page(doc)
    doc.add_heading("4. 847条允许单突与幅度分档", level=1)
    ...
    _page(doc)
    doc.add_heading("5. 从30条单突试选到19条父单突", level=1)
    doc.add_paragraph("已有30条单突试选保持为冻结上游结果。当前面板按位点覆盖要求选出19条父单突：F30、Q1、T27各保留3条，用于比较同一位置不同理化替换；其余10个位点各保留1条，以扩大位置与机制覆盖。")
    _figure(doc, figures["parent19"], "图4  19条父单突的位点配额。", 15.5)
    parent_rows = []
    for row in parents:
        mut = _mutation(row)
        parent_rows.append([
    ...
    _page(doc)
    doc.add_heading("6. 162条双突的完整序列评价", level=1)
    ...
    _page(doc)
    doc.add_heading("7. 11条双突的证据分层与全局选择", level=1)
    doc.add_paragraph("84条双突通过资格门后，采用按优先级逐项固定最优值的分类优化。该过程选择的是整体实验面板，而不是把某个连续总分最高的11条依次取出。")
    _table(doc, ["证据层", "定义", "终选数"], [
        ["A层", "NetSolP、NanoMelt、AntiFold三个家族均中等或明显有利", 3],
        ["B层", "两个家族中等或明显有利", 4],
        ["C层", "一个家族中等或明显有利，用于补充位置和机制多样性", 4],
    ], [0.7, 4.9, 0.9], 8.9)
    _point(doc, "优先增加A层和B层候选，再增加明显有利家族数。")
    _point(doc, "随后减少软序列风险和中等/明显不利的非加和残差。")
    _point(doc, "最后增加不同组成单突和不同位置覆盖；每个位置对最多1条、每个确切单突最多使用2次、每个位置最多出现3次。")
    _point(doc, "11条双突覆盖13种组成单突和10个reported位置；没有双突稳定词净新增，稳定词假设由单突T99F保留。")
    double_rows = []
    for row in doubles:
    ...
    _table(doc, ["序", "双突", "层", "ΔU", "ΔS", "ΔTm °C", "AntiFold", "入选说明"], double_rows,
           [0.3, 0.9, 0.3, 0.55, 0.55, 0.65, 0.8, 2.75], 7.5)

    _page(doc)
    doc.add_heading("8. 最终30条候选面板", level=1)
    ...
    _page(doc)
    doc.add_heading("9. 解释边界与实验建议", level=1)
    ...
    _page(doc)
    doc.add_heading("附录A  19条父单突索引", level=1)
    single_panel_rows = []
    double_panel_rows = []
    for row in panel:
    ...
    _page(doc)
    # A short spacer prevents LibreOffice from placing the appendix heading
    # above the printable top margin when it keeps the heading with the table.
    doc.add_paragraph("")
    doc.add_heading("附录B  11条双突索引", level=1)
    ...
    _add_footer(doc)
    doc.core_properties.title = "Nb252纳米抗体BL21表达量优化阶段报告"
...
def main() -> int:
    ...
    if (len(parents), len(doubles), len(panel)) != (19, 11, 30) or gate["status"] != "pass":
        raise ValueError("Final 19+11 panel inputs are incomplete or unreleased")
    figures = _make_figures(out, parents, doubles, gate)
    report = _build_report(out, parents, doubles, panel, gate, figures)
