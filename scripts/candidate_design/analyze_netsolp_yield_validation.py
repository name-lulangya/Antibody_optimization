#!/usr/bin/env python3
"""Analyze fixed NetSolP Distilled U/S scores against reported BL21 yield."""
...
from antibody_optimization.netsolp_yield import analyze_netsolp_associations  # noqa: E402
from antibody_optimization.netsolp_yield_plot import render_netsolp_yield_figure  # noqa: E402


NAMES = {
    "samples": "netsolp_yield_sample_evidence.csv",
    "metrics": "netsolp_yield_associations.csv",
    "gate": "netsolp_yield_validation_gate.json",
    "png": "netsolp_yield_validation.png",
    "svg": "netsolp_yield_validation.svg",
}
...
def main() -> int:
    ...
    sources = [
        ...
        scores / "netsolp_sample_scores.csv",
        scores / "netsolp_model_run.json",
    ]
    contract = _json(sources[1])
    model_run = _json(sources[3])
    if contract.get("status") != "pass" or model_run.get("status") != "pass":
        raise ValueError("NetSolP plan/model run status mismatch")
    for field in ("model_type", "prediction_type"):
        if model_run.get(field) != contract.get(field):
            raise ValueError(f"NetSolP plan/model run {field} mismatch")
    result = analyze_netsolp_associations(_csv(sources[0]), _csv(sources[2]))
    ...
    if any(path.exists() for path in valid.target_paths):
        raise FileExistsError("Refusing to overwrite NetSolP validation outputs")
    for path in valid.target_paths:
        path.parent.mkdir(parents=True, exist_ok=True)
    final = dict(zip((*NAMES, "summary"), valid.target_paths, strict=True))
    primary = result["primary"]
    release = {
        "weak_ranking_evidence": "ready_for_weak_netsolp_ranking_use",
        "compatibility_filter_only": "netsolp_compatibility_filter_only",
        "no_supported_use": "netsolp_not_supported_for_candidate_use",
    }[result["evidence_level"]]
    gate = {
        "schema_version": 1,
        "gate_name": "nb252_netsolp_bl21_reported_yield_validation",
        "status": "pass",
        ...
        "release": release,
        "interpretation": "Association with collaborator-reported BL21 yield; not a measured expression rate or mg/L predictor.",
    }
    with tempfile.TemporaryDirectory(prefix=".netsolp-analysis-", dir=ROOT) as temp:
        stage = Path(temp)
        ...
        _write_json(
            staged["summary"],
            {
                ...
                "counts": {
                    "samples": 47,
                    "numeric": 31,
                    "llj_ordinal_censored": 16,
                    "features": len(result["metric_rows"]),
                },
