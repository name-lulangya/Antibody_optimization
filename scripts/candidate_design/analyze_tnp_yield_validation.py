...
def main() -> int:
    ...
    if contract.get("schema_version") != 2 or contract.get("status") != "pass" or model_run.get("schema_version") != 2 or model_run.get("status") != "pass": raise ValueError("TNP V2 plan/model run status mismatch")
    ...
    gate = {"schema_version": 2, "gate_name": "nb252_tnp_bl21_reported_yield_validation_v2", "status": "pass", "generated_at": generated, "coverage": result["coverage"], "primary_feature": "tnp_psh", "expected_direction": "higher_PSH_lower_yield", "evidence_level": result["evidence_level"], "decision_reasons": result["decision_reasons"], "primary_statistics": result["primary"], "high_capacity_model_trained": False, "nb252_expression_prediction_validated": False, "selection_scope": "43_TNP_applicable_sequences_not_all_47_records", "selection_bias": "four_WCC_sequences_are_not_applicable_and_WCC_numeric_n_is_4", "release": release, "interpretation": "Association within the TNP-applicable subset of collaborator-reported BL21 yield; TNP remains a developability-risk profiler and is not a measured expression or mg/L predictor."}
    with tempfile.TemporaryDirectory(prefix=".tnp-analysis-", dir=ROOT) as temp:
        ...
        _write_json(staged["summary"], {"schema_version": 2, "status": "pass", "generated_at": generated, "python": platform.python_version(), "elapsed_seconds": round(time.perf_counter() - started, 6), "evidence_level": result["evidence_level"], "coverage": result["coverage"], "outputs": {key: str(value) for key, value in final.items() if key != "summary"}})
